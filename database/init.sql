-- F1 Dashboard Database Schema (Clean - No Sample Data)
-- database/init.sql
...
-- Teams table (will be populated by F1 processor)
CREATE TABLE teams (
    id SERIAL PRIMARY KEY,
...
-- Drivers table (will be populated by F1 processor)
...
-- F1 Sessions table (will be populated by F1 processor)
...
-- Lap times table (will be populated by F1 processor)
...
-- Telemetry data table (will be populated by F1 processor)
...
-- Race control messages (will be populated by F1 processor)
...
-- Track status (will be populated by F1 processor)
...
-- Create a single admin user for testing (no F1 data conflicts)
-- Password: admin123 (hashed with bcrypt)
INSERT INTO users (email, password_hash, first_name, last_name, is_verified) VALUES
('admin@f1dashboard.com', '$2b$10$8K4kZQxvtXgH4lQJlZ8Y1e1i8J7K6M9N0P2Q3R4S5T6U7V8W9X0Y1Z', 'Admin', 'User', true);
